...
class CopilotAutomationAndJobDetailTests(unittest.TestCase):
    def setUp(self) -> None:
        # Hermetismo: esta máquina tem chaves de LLM reais no .env — sem o patch,
        # o polimento opcional chamaria a rede e reescreveria as respostas,
        # tornando as asserções instáveis. Nos testes, o copilot é determinístico.
        patcher = unittest.mock.patch("backend.llm.available", return_value=False)
        patcher.start()
