...
def _is_ready_video_job(job_id: int) -> bool:
    """True if this job was created from a Google Drive "ready video"
    reservation (scheduler._try_create_ready_video_job) — its content is the
    user's own pre-made file, never AI-scripted, and must never be
    regenerated by the full orchestrator on retry."""
    from backend.database import SessionLocal
    from backend.models import VideoJob
    ...
    try:
        job = db.get(VideoJob, job_id)
        return bool(job and job.mode == "from_ready_video")
    finally:
        db.close()
